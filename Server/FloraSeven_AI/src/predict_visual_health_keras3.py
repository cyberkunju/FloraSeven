...
This version is compatible with Keras 3 using TFSMLayer.
"""
...
def predict_visual_health(image_path=None, image_array=None):
    ...
    # Make prediction with TFSMLayer
    # TFSMLayer returns a dictionary with the output tensor
    prediction_dict = _model(img_array)
    ...
    prediction = prediction_dict[output_key].numpy()[0][0]  # Convert to numpy and get first item
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python predict_visual_health_keras3.py <image_path>")
        sys.exit(1)
